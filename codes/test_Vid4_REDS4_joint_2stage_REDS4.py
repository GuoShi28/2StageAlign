...
def main():
    ...
    test_patch_size = 64 # if using PBM for coase alignment, the test performs on patch domain
    patch_extend = 4
    save_imgs = True
    search_window = 48
    ...
    # ----- load lightweight downsampling model -----
    # with 2 max pooling
    ...
    for subfolder_key, subfolder_GT in enumerate(subfolder_GT_l):
        ...
        for img_idx, img_path in enumerate(img_path_l):
            ...
            #----> c. batch process
            max_batch_num = (2*16)//((test_patch_size/64)**2) # can adjust based on the GPU menmory
