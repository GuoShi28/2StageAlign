...
def parse(opt_path, is_train=True):
    ...
    if is_train:
        ...
        if 'debug' in opt['name']:
            opt['train']['val_freq'] = 1e3
            opt['logger']['print_freq'] = 20
            opt['logger']['save_checkpoint_freq'] = 2e3
    else:  # test
...
def check_resume(opt, resume_iter):
    ...
    if opt['path']['resume_state']:
        ...
        opt['path']['pretrain_model_VGG'] = osp.join(opt['path']['models'],
                                                   '{}_VGG.pth'.format(resume_iter))
        logger.info('Set [pretrain_model_VGG] to ' + opt['path']['pretrain_model_VGG'])
        if 'gan' in opt['model']:
