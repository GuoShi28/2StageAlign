...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            model.optimize_parameters_rgb(current_step)
            #### log
            if current_step % opt['logger']['print_freq'] == 0:
                stop = timeit.default_timer()
                message = '[time:{:.3e}, '.format(stop - start)
            ...
            if opt['datasets'].get('val', None) and current_step % opt['train']['val_freq'] == 0:  
                ...
                if opt['dist']:
                    ...
                    for idx in range(rank, len(val_set), world_size):
                        ...
                        noise_img = visuals['LQ']
                        if noise_img.ndim == 4:
                            noise_img = util.tensor2img(noise_img[2, :, :, :])  # uint8
                        else:
                            noise_img = util.tensor2img(noise_img)
                        
                        img_dir = os.path.join(opt['path']['val_images'], str(idx))
                        util.mkdir(img_dir)
                        save_img_path = os.path.join(img_dir,
                                                    '{:s}_{:d}.png'.format(str(idx), current_step))
                        util.save_img(rlt_img[:,:,0:3], save_img_path)
                        save_img_path2 = os.path.join(img_dir,
                                                    '{:s}_{:d}_noise.png'.format(str(idx), current_step))
                        util.save_img(noise_img[:,:,0:3], save_img_path2)
                        # calculate PSNR
